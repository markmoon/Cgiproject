 ...
 int cgiMain()
{
	 ...
	 if (readnd == sizeof(struct replay_packet)) {
	 	if (replay->head.ki == KI_REPLAY) {
	 		if (replay->data == RPL_LOGIN_SUCCESS) {
	 			fprintf(cgiOut, "statuss=%d;passed=%d", 1, replay->head.extent);
	 		} else if (replay->data == RPL_LOGIN_FALLED) {
	 			 fprintf(cgiOut, "statuss=%d;passed=%d", 0, replay->head.extent);
	 		}
	 	}
	 }
	 return 0;
