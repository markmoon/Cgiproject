 ...
 int cgiMain()
{
	 ...
	 request.head.ki = KI_TELEVISION;

	 cgiHeaderContentType("text/html");
	 cgiFormInteger("television", &television, 0);
	 cgiFormInteger("mode", &mode, 0);
	 cgiFormInteger("passed", &passed, 0);
	 request.head.ttl = television;
	 ...
	 if (readnd == sizeof(struct replay_packet)) {
	 	if (replay->head.ki == KI_REPLAY) {
	 			fprintf(cgiOut, "television=%d;statuss=%d", television, replay->data);
	 	}
	 }
	 tcp_close(fd);
	 return 0;
